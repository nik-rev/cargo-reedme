...
#
# If not specified, defaults to `metadata.docs.rs.features`
#
# features = []

# Whether to pass `--all-features` to Cargo
#
# If not specified, defaults to `metadata.docs.rs.all-features`
#
# all-features = false
...
#
# If not specified, defaults to `metadata.docs.rs.rustc-args`
#
# rustc-args = []
...
You can also use `cargo reedme` as a crate. 95% of the `cargo reedme`’s logic lives in a single, pure function [`cargo_reedme::resolve`](https://docs.rs/cargo-reedme/0.4.1/cargo_reedme/fn.resolve.html)
