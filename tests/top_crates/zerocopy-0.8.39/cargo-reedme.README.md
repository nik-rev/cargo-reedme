...
- [`TryFromBytes`](https://docs.rs/zerocopy/0.8.39/zerocopy/trait.TryFromBytes.html) indicates that a type may safely be converted from
  certain byte sequences (conditional on runtime checks)
- [`FromZeros`](https://docs.rs/zerocopy/0.8.39/zerocopy/trait.FromZeros.html) indicates that a sequence of zero bytes represents a valid
  instance of a type
- [`FromBytes`](https://docs.rs/zerocopy/0.8.39/zerocopy/trait.FromBytes.html) indicates that a type may safely be converted from an
  arbitrary byte sequence
- [`IntoBytes`](https://docs.rs/zerocopy/0.8.39/zerocopy/trait.IntoBytes.html) indicates that a type may safely be converted *to* a byte
  sequence

These traits support sized types, slices, and [slice DSTs][slice-dsts].

[slice-dsts]: https://docs.rs/zerocopy/0.8.39/zerocopy/trait.KnownLayout.html
...
- [`KnownLayout`](https://docs.rs/zerocopy/0.8.39/zerocopy/trait.KnownLayout.html) indicates that zerocopy can reason about certain layout
  qualities of a type
- [`Immutable`](https://docs.rs/zerocopy/0.8.39/zerocopy/trait.Immutable.html) indicates that a type is free from interior mutability,
  except by ownership or an exclusive (`&mut`) borrow
- [`Unaligned`](https://docs.rs/zerocopy/0.8.39/zerocopy/trait.Unaligned.html) indicates that a type’s alignment requirement is 1
...
- ([try_transmute](https://docs.rs/zerocopy/0.8.39/zerocopy/macro.try_transmute.html))[`transmute`](https://docs.rs/zerocopy/0.8.39/zerocopy/macro.transmute.html) (conditionally) converts a value of
  one type to a value of another type of the same size
- ([try_transmute_mut](https://docs.rs/zerocopy/0.8.39/zerocopy/macro.try_transmute_mut.html))[`transmute_mut`](https://docs.rs/zerocopy/0.8.39/zerocopy/macro.transmute_mut.html) (conditionally) converts a
  mutable reference of one type to a mutable reference of another type of
  the same size
- ([try_transmute_ref](https://docs.rs/zerocopy/0.8.39/zerocopy/macro.try_transmute_ref.html))[`transmute_ref`](https://docs.rs/zerocopy/0.8.39/zerocopy/macro.transmute_ref.html) (conditionally) converts a
...
conversions; see the [`byteorder`](https://docs.rs/zerocopy/0.8.39/zerocopy/byteorder/) module. These types are especially useful
for network parsing.
